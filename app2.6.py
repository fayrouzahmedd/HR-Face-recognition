# Face recognition attendance (SQLite) — enrollment only via 'g' key
# Changes in this build:
#   - Removed ALL hand-gesture logic (MediaPipe & OpenCV fallbacks)
#   - Enrollment is triggered ONLY by pressing 'g'
#   - HUD/strings cleaned accordingly

import os, sys, cv2, numpy as np, time, threading, csv, shutil, re, traceback, pickle, hashlib, subprocess, sqlite3
...
CACHE_PATH     = ATTENDANCE_DIR / "emb_cache.pkl"
CACHE_ENABLED  = True
CAM_INDEX      = 0
...
class _GTTSWorker:
    ...
    def _init_backends(self):
        ...
        try:
            ...
            self._pg = None
        self._pydub = None; self._sa=None
        if self.backend is None:
            try:
                from pydub import AudioSegment
                import simpleaudio as sa
                self._pydub = AudioSegment; self._sa=sa
                self.backend = "pydub"
            except Exception:
                self._pydub=None; self._sa=None
        ...
        try:
            ...
            print("[TTS] gTTS unavailable:", e)
            self.ok_synth = False
        print(f"[TTS] backend={self.backend or 'none'} synth={'ok' if self.ok_synth else 'no'}")

    def say(self, text: str):
        if not ENABLE_TTS or not self.ok_synth:
            return
        with self.lock:
            last_text, last_ts = self.last_spoken
            if text == last_text and (time.time() - last_ts) < TTS_DEDUPE_SEC:
                return
            self.q.append(text)
            self.last_spoken = (text, time.time())

    def _synth_path(self, text: str) -> Path:
    ...
    def _synthesize(self, text: str) -> Optional[Path]:
        try:
            p = self._synth_path(text)
            if not p.exists():
                from gtts import gTTS
                gTTS(text=text, lang=TTS_LANG).save(str(p))
            return p
        except Exception as e:
    ...
    def _play(self, mp3_path: Path):
        if self.backend == "pygame" and self._pg is not None:
            try:
                self._pg.mixer.music.load(str(mp3_path))
                self._pg.mixer.music.play()
                while self._pg.mixer.music.get_busy():
                    time.sleep(0.05)
                return
            except Exception as e:
                print("[TTS] pygame play failed:", e)
        if self.backend == "pydub" and self._pydub is not None and self._sa is not None:
            try:
                audio = self._pydub.from_file(str(mp3_path), format="mp3")
                raw = audio.raw_data
                play_obj = self._sa.play_buffer(raw, num_channels=audio.channels,
                                                bytes_per_sample=audio.sample_width,
                                                sample_rate=audio.frame_rate)
                play_obj.wait_done()
                return
            except Exception as e:
                print("[TTS] pydub play failed:", e)
        if self.backend == "playsound" and self._playsound is not None:
            try:
                self._playsound.playsound(str(mp3_path))
                return
            except Exception as e:
                print("[TTS] playsound failed:", e)
        try:
            if sys.platform.startswith("win"):
                subprocess.run(["powershell","-c", f'(New-Object Media.SoundPlayer "{str(mp3_path)}").PlaySync()'], timeout=15)
            elif sys.platform == "darwin":
                subprocess.run(["afplay", str(mp3_path)], timeout=15)
            else:
                subprocess.run(["mpg123", str(mp3_path)], timeout=15)
        except Exception as e:
            print("[TTS] OS fallback failed:", e)
    ...
    def _loop(self):
        while not self.stop:
            ...
            p = self._synthesize(item)
            if p is None: continue
            self._play(p)

    def shutdown(self):
        self.stop = True
        try: self.thread.join(timeout=0.8)
...
def detect_largest_face_bbox(bgr: np.ndarray) -> Optional[Tuple[int,int,int,int]]:
    ...
    try: gray = cv2.cvtColor(bgr, cv2.COLOR_BGR2GRAY)
    except Exception: return None
    mn = _dynamic_min_size(w,h)

    faces = _detect_with_cascade(gray, FRONTAL_CASCADE, mn)
    if len(faces): return _bbox_from_faces(faces)
...
def is_crop_usable(bgr_crop: np.ndarray) -> bool:
    if bgr_crop is None or bgr_crop.size == 0: return False
    h, w = bgr_crop.shape[:2]
    if (w*h) < MIN_BBOX_AREA: return False
    g = cv2.cvtColor(bgr_crop, cv2.COLOR_BGR2GRAY)
    if cv2.Laplacian(g, cv2.CV_64F).var() < MIN_LAPLACE_VAR: return False
    return True
...
def _create_tracker(kind="KCF"):
    ...
    if hasattr(cv2, "legacy"):
        return cv2.legacy.TrackerKCF_create() if kind == "KCF" else cv2.legacy.TrackerCSRT_create()
    else:
        return cv2.TrackerKCF_create() if kind == "KCF" else cv2.TrackerCSRT_create()

def ema(prev, new, alpha): return new if prev is None else (alpha*new + (1-alpha)*prev)
...
def recognize_heavy(model_pair, db, bgr):
    ...
    if not is_crop_usable(crop): return None
...
def migrate_folder_to_sqlite_if_needed(model_pair):
    ...
    if n > 0: return
    if not AUTHORIZED_DIR.exists(): return
...
def _load_cache():
    if not CACHE_ENABLED or not CACHE_PATH.exists(): return None
    try:
...
def _manifests_equal(a,b):
    ...
    for k in a.keys():
        if len(a[k]) != len(b.get(k, [])): return False
        for (pa,ma,sa), (pb,mb,sb) in zip(a[k], b[k]):
            if pa != pb or ma != mb or sa != sb: return False
    return True
...
class CaptureWorker:
    ...
    def _run(self):
        ...
        try:
            ...
            speak("جاري إنشاء مستخدم جديد، يرجى الانتظار.")
            speak("يتم التحضير لالتقاط الصور، يرجى الثبات.")
            ...
            for idx, (stage_name, stage_count, stage_prompt) in enumerate(STAGE_LIST, start=1):
                ...
                while saved < stage_count and time.time() < deadline:
                    ...
                    pad=0.12; dx=int((x2-x1)*pad); dy=int((y2-y1)*pad)
                ...
                if idx < len(STAGE_LIST):
                    speak("يرجى الانتظار، يتم التحضير للوضعية التالية.")
                    next_stage_name, _, next_stage_prompt = STAGE_LIST[idx]
                    speak(next_stage_prompt)
...
def prompt_name(default_name: str) -> str:
    speak("من فضلك اكتب اسمك واضغط إدخال.")
    _ui_show_name_prompt_banner(NAME_PROMPT_TIMEOUT_SEC)
    entered = _timed_input(f"New user name (default '{default_name}', {NAME_PROMPT_TIMEOUT_SEC:.0f}s): ",
                           NAME_PROMPT_TIMEOUT_SEC, default_name)
...
def finalize_one(model_pair, folder_path: Path):
    ...
    if folder_path is None or not folder_path.exists(): return
    print(f"[ENROLL] finalizing {folder_path}")
    emb = embed_from_folder(model_pair, folder_path)
    if emb is None:
        print("[ENROLL] no usable embedding; removing folder")
    ...
    if dup is not None:
        ...
        if d<=ACCEPT_DIST_THRESH:
            speak(f"{name} مسجل بالفعل. سيتم التجاوز.")
            try: shutil.rmtree(folder_path)
            except Exception: pass
            return
        ...
        for ext in ("*.jpg","*.jpeg","*.png","*.webp","*.bmp"):
            for p in sorted(folder_path.glob(ext)):
                ...
                if STORE_IMAGES_IN_DB:
                    m = re.search(r"_(front|right_side|left_side)_\d+\.(jpg|jpeg|png|webp|bmp)$", p.name, re.I)
                    stg = (m.group(1) if m else "unknown")
                    db_add_image(pid, stg, crop)
                if embs_added >= MAX_TEMPLATES_PER_ID:
                    break
            if embs_added >= MAX_TEMPLATES_PER_ID:
                break
        print(f"[ENROLL] finalized in SQLite for '{final_name}' (templates: {embs_added})")
        speak(f"تم إنشاء المستخدم {final_name} بنجاح.")
        speak("سيتم تحديث قاعدة البيانات.")
    except Exception as e:
...
def finalize_all(model_pair):
    while True:
        with _pending_staging_lock:
            if not _pending_staging_queue: return
...
def enroll_allowed_now() -> Tuple[bool, str]:
    ...
    if _smoothed_dist is not None and _smoothed_dist <= ACCEPT_DIST_THRESH:
        return False, f"face already exists in DB (d={_smoothed_dist:.3f} <= {ACCEPT_DIST_THRESH})"
    if CAPTURE_ONLY_WHEN_UNAUTHORIZED:
        ...
        if _current_identity is not None:
            return False, "currently authenticated"
    return True, "ok"
        ...
        while True:
            ...
            if run_heavy_due_to_cadence and run_heavy_due_to_period:
                ...
                if rec is None:
                    ...
                    if _smoothed_dist is not None:
                        desired_label = f"{identity} d={_smoothed_dist:.2f}" if is_authorized else f"NOT AUTHORIZED d={_smoothed_dist:.2f}"
                    else:
                        desired_label = identity if is_authorized else "NOT AUTHORIZED"
                    ...
                    if _auth_streak >= STABLE_FRAMES_AUTH and _current_identity is not None:
                        ...
                        if (time.time() - _last_auth_time.get(_current_identity, 0.0)) >= 45.0:
                            if datetime.now().hour < 18:
                                speak(f"مرحباً، {_current_identity}.")
                                speak(f"تم منح الوصول لـ {_current_identity}.")
                                if log_event(_current_identity, "Arrival", _smoothed_dist):
                                    speak("تم تسجيل الحضور.")
                            else:
                                speak(f"إلى اللقاء، {_current_identity}.")
                                if log_event(_current_identity, "Departure", _smoothed_dist):
                                    speak("تم تسجيل الانصراف.")
                            _last_auth_time[_current_identity] = time.time()
                    ...
                    try:
                        ...
                        _tracker=None; _tracker_bbox=None
            else:
                # tracker update
                if _tracker is not None and _tracker_bbox is not None:
                    ...
                    if ok_tr:
                        ...
                        _smoothed_bbox = ema_bbox(_smoothed_bbox,(x,y,x+w,y+h),BBOX_SMOOTH_ALPHA)
                        ...
                        cv2.putText(frame_disp,_last_draw_label,(sx1+3,sy2+th+3),FONT,0.6,(255,255,255),2)
                        _hold_counter = max(0,_hold_counter-1)
                    else:
                        _tracker = None
                        _tracker_bbox = None
            ...
            with _ui_lock:
                ...
                if _name_prompt_banner_until > now_wall:
                    msg = "من فضلك اكتب اسمك في نافذة الأوامر ثم اضغط Enter..."
            ...
            cv2.putText(frame_disp, "Press 'g' to enroll", (10, frame_disp.shape[0]-12), FONT, 0.6, (0,255,255), 2, cv2.LINE_AA)

            cv2.imshow("Face System (SQLite, press 'g' to enroll)", frame_disp)

            key = (cv2.waitKey(1) & 0xFF)
            if key == ord('g') and not _capture_in_progress:
                ...
                if allowed:
                    if capt.start_capture():
                        ...
                        print("[MANUAL] worker busy.")
                else:
                    print("[MANUAL] capture blocked:", reason)
                    speak("لا يمكن إنشاء مستخدم جديد الآن.")
            if key == ord('q'):
                break

            # finalize pending enrollments + DB reload
